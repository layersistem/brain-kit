...
# hour has passed) and tomorrow's. Done: change `@due` to `@due✓` on that line - the hook stops seeing it.
# Silent when nothing is due. Companion of time-inject.sh: the clock says what time it is, this says
# what that time means for you. Config: BRAIN_ROOT/BRAIN_DIR/BRAIN_INSTANCE as in _focus_inject.sh.
...
[ -n "$OUT" ] || exit 0
echo "DUE ($I; mark done by changing @due to @due✓; format: '@due YYYY-MM-DD[ HH:MM] text'):"
printf '%s\n' "$OUT"
