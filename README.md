...
Two features are opt-in and `setup.sh` asks before installing them. An empty answer means no, and so
does a non-interactive run (stdin not a terminal) without the matching flag:

```
Narrow the context window? This sets CLAUDE_CODE_AUTO_COMPACT_WINDOW in the project's .claude/settings.json
so warnings and auto-compact trigger earlier (recommended value 500000). [y/N]
Install self-compact (requires tmux; the agent compacts itself at the hard threshold)? [y/N]
```

The first writes the window into `<project>/.claude/settings.json` (`env`), where both Claude Code and
the context hook read it; the suggested value is what most of the author's own projects run with. The
...
`--context-window=<N|no>`, `--self-compact=<session|no>`, `--yes-defaults` (every question takes its
default, no prompt). `scripts/update.sh` never changes either choice.

Two things install on their own when the machine can carry them. The **index sweeper** timer
(`scripts/systemd/brain-index-sweep.*`, every 2 minutes) embeds what the write hook only chunked; without
`systemd --user` the installer prints the cron line instead, and `--no-timers` skips the step. The **desk
...
| `BRAIN_RECALL_REPEAT_FILTER` | `1` | within a session, drop a note already shown once when it is only FAIR and shares no word with the prompt, or when it is a handover-style note (hub, handoff, closing, compact) that has never been opened. First showing is never filtered, shared docs are never filtered, and a fully filtered turn prints nothing rather than claiming no match; `0` restores pre-1.1.0 behaviour |
